...
import { renderRowValue } from "../renderRowValue";
...
export function ResourceTable<ResourceType extends WithId = any>(props: Props<ResourceType>) {
  // Is table waiting for data
  // Css classes
  const classes = useResourceTableStyles();
  const openCreateForm = useOpenCreateForm();
  const { data, loading, onDelete, pg } = useResource<ResourceType>(props.transform);
    ...
    <Paper className={classes.root} elevation={4}>
      ...
      <div className={classes.tableWrapper}>
        <Table size="small" className={classes.table}>
          ...
          <TableBody>
            {data.map((row, j) => (
              <TableRow key={row?.id ?? j} selected={selection.selected[row.id]}>
                ...
                <TableCell padding="none">
                  <Box pl={1}>
                    <Checkbox
                      ...
                      onChange={(_, checked) => selection.changeOne(row.id, checked)}
                    />
                  </Box>
                </TableCell>
                {columns.map((col, i) => (
                  <TableCell align={col.align} key={`${row.id}${i}`} component="th" scope="row">
                    <div>{col.render?.(row) ?? renderRowValue(col, row)}</div>
                  </TableCell>
                ))}
              </TableRow>
            ))}
