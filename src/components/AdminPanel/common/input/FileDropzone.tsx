...
interface Props<T = any> {
  // files: ReturnType<typeof useState>;
  files: [T, (val: T) => any];
}

/** Wrapper around FileDropzone */
export function useFileDropzone(
  props: { files: PreviewFile[] } = { files: [] },
): [() => JSX.Element, PreviewFile[]] {
...
function FileDropzone({ files: [files, setFiles] }: Props<PreviewFile[]>) {
  const snackbar = useSnackbar();

  const onDrop = (accepted: File[], rejected: File[]): void => {
    ...
    const newFiles: PreviewFile[] = accepted.map(file => ({
      file,
  ...
  return (
    <div>
      ...
      {files.length > 0 && (
        <p className="text-center pt-4 text-lg">Added files (you can reorder them latter)</p>
      )}
      <Box
        py={3}
        style={{
